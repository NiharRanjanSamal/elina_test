...
BEGIN
    ...
    VALUES 
        (@TenantId, @TaskId, '2025-11-05', @PlannedQty, @PlannedQty, @PlannedQty, 'Day 1: On track', 1, @UserId, GETDATE(), @UserId, GETDATE()),
        (@TenantId, @TaskId, '2025-11-06', @PlannedQty, @PlannedQty, @PlannedQty, 'Day 2: On track', 1, @UserId, GETDATE(), @UserId, GETDATE()),
        (@TenantId, @TaskId, '2025-11-07', @PlannedQty, @PlannedQty, @PlannedQty, 'Day 3: On track', 1, @UserId, GETDATE(), @UserId, GETDATE())
    WHERE NOT EXISTS (SELECT 1 FROM dbo.task_updates WHERE tenant_id = @TenantId AND task_id = @TaskId AND update_date = '2025-11-05');
    
    -- Day 4-5: actual < plan
    INSERT INTO dbo.task_updates (tenant_id, task_id, update_date, planned_qty, actual_qty, daily_update_qty, remarks, activate_flag, created_by, created_on, updated_by, updated_on)
    VALUES 
        (@TenantId, @TaskId, '2025-11-08', @PlannedQty, @PlannedQty * 0.8, @PlannedQty * 0.8, 'Day 4: Slight delay', 1, @UserId, GETDATE(), @UserId, GETDATE()),
        (@TenantId, @TaskId, '2025-11-09', @PlannedQty, @PlannedQty * 0.75, @PlannedQty * 0.75, 'Day 5: Behind schedule', 1, @UserId, GETDATE(), @UserId, GETDATE())
    WHERE NOT EXISTS (SELECT 1 FROM dbo.task_updates WHERE tenant_id = @TenantId AND task_id = @TaskId AND update_date = '2025-11-08');
    ...
    INSERT INTO dbo.task_updates (tenant_id, task_id, update_date, planned_qty, actual_qty, daily_update_qty, remarks, activate_flag, created_by, created_on, updated_by, updated_on)
    VALUES 
        (@TenantId, @TaskId, '2025-11-11', @PlannedQty, @PlannedQty * 0.9, @PlannedQty * 0.9, 'Day 7: Catching up', 1, @UserId, GETDATE(), @UserId, GETDATE()),
        (@TenantId, @TaskId, '2025-11-12', @PlannedQty, @PlannedQty * 0.95, @PlannedQty * 0.95, 'Day 8: Progressing', 1, @UserId, GETDATE(), @UserId, GETDATE()),
        (@TenantId, @TaskId, '2025-11-13', @PlannedQty, @PlannedQty * 0.85, @PlannedQty * 0.85, 'Day 9: Steady progress', 1, @UserId, GETDATE(), @UserId, GETDATE())
    WHERE NOT EXISTS (SELECT 1 FROM dbo.task_updates WHERE tenant_id = @TenantId AND task_id = @TaskId AND update_date = '2025-11-11');
